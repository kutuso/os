#!/usr/bin/env bash
# Interactive: boot the latest ISO (or the installed disk) in QEMU.
# On a graphical host it opens a window; headless it serves VNC on
# 127.0.0.1:5900+N (ssh -L from another machine to view it).
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ "$BOOT" = "disk" ]; then
  [ -f work/test-vm-disk.raw ] || { echo "KUTU_VM_BOOT=disk needs a disk; run once without it, or set KUTU_VM_DISK"; exit 1; }
fi
